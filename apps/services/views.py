...
def service_access(request, token):
    ...
    # If service has a URL, redirect to it with the token so your internal app
    # can call back /services/verify/<token>/ to confirm identity
    if us.service.url:
        from urllib.parse import urlparse, urlencode, urlunparse, parse_qs
        parsed = urlparse(us.service.url)
        redirect_url = us.service.url
        # Append token as query param if not already there
        separator = '&' if '?' in redirect_url else '?'
        redirect_url = f"{redirect_url}{separator}nexatoken={token}"
        from django.shortcuts import redirect as dj_redirect
        return dj_redirect(redirect_url)

    # No URL → show a service info page
